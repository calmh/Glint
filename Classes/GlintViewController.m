...
- (void)viewDidLoad {
	...
	NSString* filename = [NSString stringWithFormat:@"%@/track-%@.gpx", documentsDirectory, [[NSDate date] description]];
        ...
        statusLabel.text = [NSString stringWithFormat:@"Glint %@(%@)", marketVer, bundleVer];
...
- (bool)precisionAcceptable {
        static double minPrec = 0.0;
        if (minPrec == 0.0)
                minPrec = USERPREF_MINIMUM_PRECISION;
        double currentPrec = locationManager.location.horizontalAccuracy;
...
- (void)takeAveragedMeasurement:(NSTimer*)timer
{
        ...
        if (![self precisionAcceptable])
                return;
        
        averagedMeasurements++;
        
        if (last)
                totalDistance += [last getDistanceFrom:current];
        [last release];
        last = current;
        [last retain];
...
- (void)updateDisplay:(NSTimer*)timer
{
        ...
        bool stateGood = [self precisionAcceptable];
        ...
        if (stateGood) {
                ...
                if (current.speed >= 0.0)
                        currentSpeedLabel.text = [NSString stringWithFormat:speedFormat, current.speed*speedFactor];
                else
                        currentSpeedLabel.text = @"?";
                ...
                if (current.course >= 0.0)
                        compass.course = current.course;
                else
                        compass.course = 0.0;
